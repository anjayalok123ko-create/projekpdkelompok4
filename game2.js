...
function explodeBomb(bomb) {
  ...
  // MODIFIKASI TERBARU: SEKALI MENANG (HABISKAN ENEMIES LANGSUNG TRIGGER VICTORY)
...
function triggerGameOver(manual = false) {
  ...
  if (!manual) {
    saveScore();
    document.getElementById('go-score').innerHTML = `FINAL SKOR AKHIR: ${gameState.score}<br>WAKTU BERTAHAN: ${gameState.timeElapsed}s`;
...
function triggerVictory() {
  ...
  saveScore();
  document.getElementById('vc-score').innerHTML = `TOTAL SKOR AKHIR: ${gameState.score}<br>TOTAL WAKTU: ${gameState.timeElapsed}s`;
